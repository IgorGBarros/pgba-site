...
const SERVICES = [
  {
    ...
    description:
      'Construímos pipelines ETL entre seus bancos de dados (SAP, ERPs) e dashboards executivos em tempo real com Power BI e Python.',
    features: [
      'Modelagem P&L e Fluxo de Caixa',
      'Extração de dados SAP / ERP',
  ...
  },
  {
    ...
    description:
      'Redução de custos operacionais substituindo entrada manual de dados por scripts Python e workflows Power Automate.',
    features: [
      'Scripts de Automação Python',
      'Workflows Supply Chain & MRO',
      'Integrações via API',
    ],
    color: 'violet',
  ...
  },
  {
    ...
    description:
      'Desenvolvimento fullstack end-to-end. Construímos SaaS internos, portais e integramos modelos de IA treinados nos seus dados.',
    features: [
      'React / Node / Python',
...
const TIMELINE = [
  {
    icon: Factory,
    period: 'Indústria Petroquímica',
    role: 'Automação & Manutenção',
    description:
      'Desenvolvimento de automações, relatórios gerenciais e análise de dados nos setores de manutenção, materiais e suprimentos.',
  ...
  {
    icon: Globe,
    period: 'Inovação & Pesquisa',
    role: 'Desenvolvedor FullStack',
    description:
      'Participação em projeto de startup com apoio do CNPq e UFBA, desenvolvendo soluções web completas.',
    color: 'text-blue-400',
    bg: 'bg-blue-500/10',
  ...
  {
    icon: LineChart,
    period: 'Controladoria & Finanças',
    role: 'Especialista em BI',
    description:
      'Desenvolvimento de Business Intelligence e análise de dados para suporte à tomada de decisão estratégica.',
...
const CASES = [
  {
    sector: 'Indústria Petroquímica',
    title: 'Sistema de Apoio à Decisão',
    description:
      'Automação de relatórios, análise de dados e indicadores de manutenção para otimização de processos industriais.',
    tags: ['Python', 'Power BI', 'SAP'],
    icon: Factory,
    color: 'cyan',
  },
  {
    sector: 'Setor Financeiro',
    title: 'Plataforma de BI Gerencial',
    description:
      'Dashboards executivos para P&L, fluxo de caixa e indicadores de controladoria com atualização em tempo real.',
  ...
  },
  {
    ...
    description:
      'Workflows automatizados com Power Platform, integrações SAP e eliminação de processos manuais repetitivos.',
    tags: ['Power Automate', 'SAP', 'RPA'],
    icon: Cpu,
    color: 'blue',
  },
];
...
const MINHA_AMORA_FEATURES = [
  {
    ...
    description: 'Controle completo do seu estoque com alertas automáticos e sugestões de reposição.',
  },
  {
    icon: TrendingUp,
    title: 'Apoio à Decisão',
    description: 'Saiba exatamente o que comprar, quando comprar e quanto investir baseado nos seus dados.',
  },
  {
    icon: Users,
    title: 'Feito para Consultoras',
    description: 'Interface simples e intuitiva, pensada para quem não é técnica mas precisa de controle.',
  },
  {
    icon: Sparkles,
    title: 'Diferente de Tudo',
    description: 'Não é planilha, não é genérico. É inteligência real para o seu negócio de revenda.',
  },
];
...
export default function PGBASite() {
  ...
  return (
    <div
      className={`min-h-screen font-outfit selection:bg-cyan-500/30 overflow-x-hidden transition-colors duration-500 ${
        isDarkMode
    ...
    >
      ...
      {/* ════════════════════════════════════════════
          NAVBAR STICKY (aparece após scroll)
         ════════════════════════════════════════════ */}
      <nav
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <a href="#" className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span
                className={`text-[10px] sm:text-xs uppercase tracking-wider font-medium hidden sm:block ${
                  isDarkMode ? 'text-slate-400' : 'text-slate-500'
                }`}
              >
          ...
          >
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="hover:text-cyan-500 transition-colors duration-200 relative group"
              >
                {link.label}
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="#contato"
              className="hidden sm:inline-flex px-4 py-2 sm:px-6 sm:py-2.5 text-xs sm:text-sm font-bold bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full hover:from-cyan-400 hover:to-blue-400 transition-all duration-200 shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 hover:scale-105"
            >
              Fale Conosco
            </a>
            ...
            {TIMELINE.map((item, i) => (
              <motion.div
                ...
                className={`relative p-6 rounded-2xl border backdrop-blur-sm transition-all duration-300 group ${
                  isDarkMode
                    ? `bg-slate-900/50 ${item.border} hover:bg-slate-800/50`
              ...
              >
                ...
                <div
                  className={`text-xs font-bold uppercase tracking-wider mb-1 ${item.color}`}
                >
                  {item.period}
                </div>
...
{/*
              // ══════════════════════════════════════════════════════════
// PGBASite.tsx — PARTE 2 (continuação exata de onde parou)
// Cole este código IMEDIATAMENTE após o corte da Parte 1
// ══════════════════════════════════════════════════════════
*/}
              Serviços
            </motion.div>
            ...
            {SERVICES.map((service, i) => (
              <motion.div
                ...
                className={`group p-6 sm:p-8 rounded-2xl sm:rounded-3xl border backdrop-blur-sm transition-all duration-300 ${
                  isDarkMode
                    ? `bg-slate-900/50 border-slate-800/50 ${service.hoverBorder} hover:bg-slate-800/50`
                    : `bg-white/80 border-slate-200 hover:border-${service.color}-400/50 hover:shadow-xl`
                }`}
              >
                <div
                  className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br ${service.gradient} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 ring-1 ${service.ring}`}
                >
                  <service.icon
                ...
                <ul className="space-y-2">
                  {service.features.map((feature, j) => (
                    ...
                    >
                      <CheckCircle
                        className={`w-3 h-3 shrink-0 ${service.checkColor}`}
                      />
                      {feature}
                    </li>
                  ))}
            ...
            {CASES.map((caseItem, i) => (
              <motion.div
                ...
                className={`group relative p-6 sm:p-8 rounded-2xl border overflow-hidden transition-all duration-300 ${
                  isDarkMode
                    ? `bg-slate-900/50 border-slate-800/50 hover:border-${caseItem.color}-500/50 hover:bg-slate-800/50`
                    : `bg-white/80 border-slate-200 hover:shadow-xl hover:border-${caseItem.color}-300`
                }`}
              >
                {/* Glow de fundo */}
                <div
                  className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-${caseItem.color}-500/5 to-transparent blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                />
                ...
                <div className="relative z-10">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${
                      isDarkMode
                        ? `bg-${caseItem.color}-500/10`
                        : `bg-${caseItem.color}-50`
                    }`}
                  >
                    <caseItem.icon
                      className={`w-6 h-6 ${
                        isDarkMode
                          ? `text-${caseItem.color}-400`
                          : `text-${caseItem.color}-500`
                      }`}
                    />
                  </div>
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              {/* Info de Contato */}
              <div
                className={`flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 mt-8 sm:mt-12 pt-6 sm:pt-8 border-t ${
      ...
      {/* ════════════════════════════════════════════
          FOOTER — COM FRASE DE PROTEÇÃO JURÍDICA
         ════════════════════════════════════════════ */}
      <footer
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span
                  className={`text-xs uppercase tracking-wider font-medium ml-2 hidden sm:inline ${
                    isDarkMode ? 'text-slate-500' : 'text-slate-400'
                  }`}
                >
                  Solutions Technologies
  ...
  {/*
 // ══════════════════════════════════════════════════════════
// PGBASite.tsx — PARTE 3 (FINAL)
// Cole IMEDIATAMENTE após o corte "FRASE DE PROTEÇÃO JUR"
// ══════════════════════════════════════════════════════════

          {/* ═══ FRASE DE PROTEÇÃO JURÍDICA (OBRIGATÓRIA) ═══ */}
          <div
