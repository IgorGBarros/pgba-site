...
import {
  ...
  AlertTriangle, Layers, Brain, Loader2, Target, Trophy, Percent
} from "lucide-react";
import {
...
export default function Dashboard() {
  ...
  // 🚀 TOP 3 PRODUTOS VENDIDOS NO MÊS
  ...
  const expiringSoon = items.flatMap(item => {
      ...
      return batches.filter((b: any) => {
          if (!b.expiration_date || b.quantity <= 0) return false;
    ...
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-20 border-b border-border bg-card">
        <div className="mx-auto flex max-w-5xl items-center gap-3 px-6 py-4">
          ...
          <h1 className="font-display text-lg font-bold text-foreground">Dashboard Analítico</h1>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            ...
            { label: "Próximos do Vencimento", value: String(expiringSoon.length), icon: AlertTriangle, pro: false, color: "text-orange-500" },
          ].map((s) => (
            <div key={s.label} className="relative rounded-xl border border-border bg-card p-5 transition-shadow hover:shadow-md">
              ...
              <p className={`font-display text-2xl font-bold text-foreground ${s.pro && isLocked("dashboard_kpi_advanced") ? "blur-sm select-none" : ""}`}>
                {loading ? <span className="animate-pulse text-muted-foreground/50">...</span> : s.value}
              </p>
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList className="w-full justify-start overflow-x-auto bg-transparent p-0">
            <TabsTrigger value="overview" className="text-xs data-[state=active]:bg-primary data-[state=active]:text-white rounded-full px-4 py-2 border mr-2">
              Visão Geral
            </TabsTrigger>
            <TabsTrigger value="ai" className="text-xs data-[state=active]:bg-primary data-[state=active]:text-white rounded-full px-4 py-2 border" onClick={() => { if (!myAnalytics && !myAnalyticsLoading) fetchMyAnalytics(); }}>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="rounded-xl border border-border bg-card p-5 relative overflow-hidden">
              <h2 className="font-display text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
                <Trophy className="h-4 w-4 text-amber-500" /> Top 3 Mais Vendidos (Neste Mês)
              </h2>
              {loading ? (
                ...
                <div className="space-y-2">
                  {topProducts.map((p, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-secondary/20 p-3 rounded-lg border border-border/50">
                      ...
                      <span className="font-mono text-sm font-bold text-primary">{formatMoney(p.revenue)}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground text-center py-6">Nenhuma venda registrada este mês.</p>
              )}
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="relative rounded-xl border border-border bg-card p-5">
                <h2 className="font-display text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
                  <Layers className="h-4 w-4 text-primary" /> Distribuição por Categoria
                </h2>
                {pieData.length > 0 ? (
              ...
              <div className="relative rounded-xl border border-border bg-card p-5">
                <h2 className="font-display text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
                  <DollarSign className="h-4 w-4 text-primary" /> Valor Investido por Categoria
                </h2>
                {barData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={barData}>
                      ...
                      <Tooltip formatter={(v: number) => formatMoney(v)} cursor={{fill: 'transparent'}}/>
      ...
      <UpgradeModal
        ...
        description="Acesse o ranking de vendas, ticket médio e gráficos detalhados."
      />
    </div>
  );
}