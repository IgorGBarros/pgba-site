...
export default function StockWizard() {
  ...
  const handleBarcodeInput = async (barcode: string) => {
    ...
    try {
      ...
      if (existing) {
        ...
        // 🚀 REMOVIDO: setStep(1) para o usuário ver o nome preenchido antes de avançar
        return;
      }
    ...
    try {
      ...
      if (result.product) {
        ...
        toast({ title: "Produto identificado!", description: "Dados preenchidos." });
      }
    } catch {
      // No match anywhere
    }
    setLookupLoading(false);
    // 🚀 REMOVIDO: setStep(1) para permitir edição manual do nome
  };

  const handleBarcodeScan = async (barcode: string) => {
    setShowScanner(false);
    await handleBarcodeInput(barcode);
    // 🚀 Se vier da CÂMERA, ele avança sozinho por comodidade
    setStep(1);
  };
  ...
  const canAdvance = () => {
    // 🚀 ATUALIZADO: Agora exige que o nome do produto esteja preenchido para avançar do Step 0
    if (step === 0) return !!data.barcode?.trim() && !!data.product_name?.trim() && !lookupLoading;
            ...
            <motion.div key="scan" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              {showScanner ? (
                ...
                <div className="space-y-6 rounded-xl border border-border bg-card p-5">
                  ...
                  {/* 🚀 AUTO-COMPLETE DO NOME IMPLEMENTADO */}
                  {data.barcode && !lookupLoading && (
