...
export default function ProductSearchModal({ isOpen, onClose, onSelect }: ProductSearchModalProps) {
  ...
  // Fetch catalog from Django on open
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/products/`)
      .then((r) => r.json())
      .then((data: any[]) => {
        const mapped: GlobalProduct[] = data.map((p) => ({
          id: p.id,
          name: p.name,
          sku: p.natura_sku || null,
          barcode: p.bar_code || "",
          category: p.category || "",
          official_price: p.official_price ? parseFloat(p.official_price) : null,
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-secondary/30">
            {loading ? (
              <div className="py-20 flex flex-col items-center gap-3">
                ...
                <span className="text-sm font-medium text-muted-foreground">Carregando catálogo...</span>
              </div>
            ) : results.length === 0 ? (
              <div className="py-20 text-center flex flex-col items-center">
                <Package className="w-12 h-12 mb-3 text-muted-foreground/20" />
                <p className="font-medium text-muted-foreground">
                  {query.length > 1 ? "Nenhum produto encontrado." : "Digite ao menos 2 caracteres."}
                </p>
                <p className="text-xs mt-1 text-muted-foreground/70">Busca por nome ou SKU no catálogo.</p>
              </div>
            ) : (
              results.map((item) => (
                <button
                ...
                >
                  ...
                  <div className="self-center">
                    <ChevronRight className="text-muted-foreground/30 group-hover:text-primary" size={20} />
                  </div>
                </button>
              ))
            )}
